...
export async function createAndLaunchAgent(
    ...
    await sandbox.commands.run(
        `nohup bash -c 'cd /workspace/data && WORKSPACE_DIR=/workspace/data CALLBACK_URL="${callbackUrl}" CALLBACK_URL_BASE="${baseUrl}" RESUME_SESSION_ID="${sessionId || ""}" ANTHROPIC_API_KEY="${process.env.ANTHROPIC_API_KEY || ""}" npx tsx /app/agent.mts < /tmp/agent_input.txt >> /tmp/agent_stdout.log 2>> /tmp/agent_stderr.log' &>/dev/null &`
    );
